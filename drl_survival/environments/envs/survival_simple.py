...
class SurvivalEnvSimple(gym.Env):
    ...
    def __init__(self, render_mode = None, view_size=512, world_size=1024):
        ...
        self.val_map = {
            "x":0, "y":1, "radius":2,"velocity":3,"bearing":4,"angular_velocity":5,"thrust":6,"energy":7,"closest_food_dist":8,
            "pos":[0,1]
        }
        ...
        # Box takes a list of lows, highs
        # dot product to nearest food, cross product to nearest food, distance from nearest food, velocity, angular_vel, thrust
        self.observation_space = spaces.Box(np.array([-1.,-1.,0.,0.,0.,0.]), np.array([1.,1.,float(world_size*2.),self.max_vel, self.max_ang_vel, self.max_thrust]),shape=(6,), dtype=np.float32)
    ...
    def _init_state(self):
        ...
        self._state = {
            # agent state: [x,y,radius,vx,vy,bearing,angular_v,thrust, energy, closest_food_dist]
            "agent":np.array([agent_init_pos[0],agent_init_pos[1],15.0,0.,0.,0.,0.,self.max_energy,0]),
        ...
        closest_food_dist = 0
        self.closest_food = self.get_closest_food(agent_init_pos)
        if self.closest_food is not None:
            closest_food_dist = np.linalg.norm(self.closest_food - agent_init_pos)
        self.set_agent_val("closest_food_dist", closest_food_dist)
    ...
    def _render(self, output_frame = False):
        ...
        if self.closest_food is not None:
            ...
            pygame.draw.line(
                ...
                0,
                tuple(agent_to_view),
                tuple(self.closest_food + world_to_view),
                width=2,
            )
    ...
    def get_closest_food(self, pos):
        ...
        for bin_x in range(int(self.world_size / self.bin_size)):
            for bin_y in range(int(self.world_size / self.bin_size)):
                food = self._state["food"][bin_x*self.bins + bin_y]    
                # resolve food 
                for f in range(len(food)):
                    dist = np.linalg.norm(food[f]-pos)
    ...
    def step(self, action):
        ...
        if not truncated and not terminated:
            ...
            closest_food_dist_prev = self.get_agent_val("closest_food_dist")
            self.closest_food = self.get_closest_food(next_pos)
            closest_food_dist_next = 0
            d_closest_food_dist = 0
            if self.closest_food is not None:
                closest_food_dist_next = np.linalg.norm(self.closest_food - next_pos)
                # don't penalize for jump in distance to closest food on a step we ate food
                if food_eaten == 0:
                    d_closest_food_dist = closest_food_dist_prev - closest_food_dist_next
            self.set_agent_val("closest_food_dist",closest_food_dist_next)
            ...
            # reward is getting closer to nearest food + bonus for finding food
            reward = food_eaten * 100 + d_closest_food_dist
    ...
    def _get_obs(self):
        # dot to nearest food, cross to nearest food, distance from nearest food, velocity, thrust, ang vel
        pos = self.get_agent_val("pos")
        bearing = self.get_agent_val("bearing")
        forward_vec = np.array([np.cos(bearing),np.sin(bearing)])
        
        dot_to_food = -1
        cross_to_food = -1
        closest_food_dist = 1000
        to_nearest_food = normalize(self.closest_food-pos)
        if self.closest_food is not None:            
            dot_to_food = np.dot(forward_vec, to_nearest_food)
            cross_to_food = np.sign(np.cross(to_nearest_food, forward_vec))
            closest_food_dist = np.linalg.norm(self.closest_food-pos)
            
        return np.array([dot_to_food, cross_to_food, closest_food_dist, self.get_agent_val("velocity"), self.get_agent_val("thrust"), self.get_agent_val("angular_velocity")])
